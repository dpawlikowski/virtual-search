...
- The default search now includes literal substrings inside words while keeping MiniSearch fuzzy and prefix results. A query such as `ra` finds and highlights the fragment in `rapid`, `paragraph`, and `vulnerability`.
- Email search in the demo now includes the sender field. News search includes the source and category fields. All visible searchable fields can be highlighted.
- Search option normalization now lives in core through `mergeSearchOptions`. Rapid consecutive option updates no longer lose state, and regex remains mutually exclusive with exact match.
- Added a search behavior matrix covering default search, regex, exact match, whole word, case sensitivity, Unicode, punctuation, phrases, multiple fields, repeated matches, invalid expressions, and fuzzy typos.
