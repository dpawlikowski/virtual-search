...
- **Fuzzy/prefix highlighting bled into unrelated words.** Fuzzy/prefix search matches on whole-word *tokens* (MiniSearch tokenizes the text and reports the matched tokens verbatim), but `resolveRanges` re-located those tokens as raw substrings — so searching `on` highlighted the `on` *inside* `onboarding`, `conversation`, `hydration`, etc., while the real word was lost in the noise. Fuzzy/prefix highlights (and the `MatchMinimap` tooltip snippet) are now word-bounded via a new `isHighlightWholeWord` policy; exact-match still highlights substrings unless `wholeWord` is set, and regex still bounds via the pattern. The word boundary is Unicode-aware (`\p{L}\p{N}` lookarounds) rather than the ASCII-only `\b`, which also fixes `wholeWord` matching/highlighting of accented words like `café` (previously `\b` refused to bound them, so an exact whole-word search for an accented term found nothing)
