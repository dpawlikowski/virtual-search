...
describe('demo search highlighting (as the demo wires it)', () => {
  ...
  // any of them is a fragment carved out of a larger word.
  function demoHighlights(items: VirtualItem[], fields: string[], query: string) {
    ...
    for (const match of searchItems(idx, query, map)) {
      ...
      for (const [field, terms] of match.terms) {
        ...
        for (const r of resolveRanges(val, terms, false, wholeWord)) {
          ...
          if (/[\p{L}\p{N}]/u.test(before) || /[\p{L}\p{N}]/u.test(after)) anyFragment = true
        }
      }
    }
  ...
  const EMAIL_FIELDS = ['subject', 'preview']
  const NEWS_FIELDS = ['headline', 'body']

  it('never highlights a fragment inside a larger word (emails)', () => {
    for (const q of ['on', 'race', 'deployment', 'review', 'the', 'RFC']) {
      const { anyFragment, painted } = demoHighlights(emails, EMAIL_FIELDS, q)
      expect(painted.length, `expected matches for "${q}"`).toBeGreaterThan(0)
      expect(anyFragment, `"${q}" bled into a larger word: ${painted.join(', ')}`).toBe(false)
    }
  })

  it('never highlights a fragment inside a larger word (news)', () => {
    for (const q of ['AI', 'on', 'code', 'source', 'security']) {
      const { anyFragment, painted } = demoHighlights(news, NEWS_FIELDS, q)
      expect(painted.length, `expected matches for "${q}"`).toBeGreaterThan(0)
      expect(anyFragment, `"${q}" bled into a larger word: ${painted.join(', ')}`).toBe(false)
    }
  })
